...
class CarsFixClass(models.Model):
    ...
    car = fields.Many2one('car.data', string='Car Information', domain="[('customer','=',customer)]",
    ...
    entery_date = fields.Datetime(string='Entry date', track_visibility='onchange', readonly=True)
    fix_date = fields.Date(string='Last Maintenance date', track_visibility='onchange',
                           compute='cal_last_maintenance_date')
    delivery_date = fields.Date(string='Finish date', track_visibility='onchange',readonly=True)
    ...
    car_issue = fields.Many2many('car.issue', string='Car Issue')

    service = fields.One2many('worker.service', 'service_inv', string='Service', track_visibility='onchange',
                              store=True)
    ...
    location_id = fields.Many2one('stock.location', string='Source Location	', track_visibility='onchange',
                                  related='operation_type.default_location_src_id')
    location_dest_id = fields.Many2one('stock.location', string='Destination Location', track_visibility='onchange',
                                       related='operation_type.default_location_dest_id')

    counter = fields.Integer(default=1, store=True, copy=True)

    @api.multi
    @api.constrains('entery_date', 'fix_date')
    def check_fixing_times_in_month(self):
        for rec in self:
            if rec.fix_date:
                current = fields.Date.today()
                operation = int(str((current - rec.fix_date).days))
                print(operation)
                # if self.date_to:
                #     fmt = '%Y-%m-%d'
                #     d1 = self.date_from
                ...
                #     self.training_period = str(int((int(days_between_dates))))
                if operation <= 30:
                    raise ValidationError('Car has fixed within 30 days!!!')

    @api.multi
    @api.depends('state', 'customer', 'chassis_number')
    def cal_last_maintenance_date(self):
        for line in self:
            asd = self.env['car.fix'].search(
                [('customer', '=', line.customer.id), ('chassis_number', '=', line.chassis_number),
                 ('state', 'in', ('invoice', 'finish'))])
            if asd:
                list = []
                for rec in asd:
                    if rec.id != line.id:
                        if rec.entery_date:
                            d = dateutil.parser.parse(str(rec.entery_date)).date()
                            list.append(d)
                print(list)
                # print(max(list))
                if list == []:
                    line.fix_date = fields.Date.today()
                    print('shit')

                else:
                    line.fix_date = max(list)
            else:
                line.fix_date = fields.Date.today()
    ...
    def create_delivery_records_in_the_stock(self):
        self.entery_date = fields.Datetime.now()
        ...
        for line in self:
            asd = self.env['stock.picking'].search(
                [('partner_id', '=', line.customer.id), ('supervisor_name', '=', line.supervisor_name),
                 ('technician_name', '=', line.technician_name), ('plate_number', '=', line.plate_number),
                 ('car_id', '=', line.id), ('counter', '=', line.counter)])
            if asd:
                for rec in asd:
...
class WorkerServiceClass(models.Model):
    ...
    product = fields.Many2one('product.product', string='Service', required=True, domain=[('service_ok', '=', True)])
    worker = fields.Char('Worker')
    no_of_hours = fields.Float('Time(H:M)')
